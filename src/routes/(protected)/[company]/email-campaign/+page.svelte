<script>
	...
	import { emailSchema, referenceSchema } from '$lib/config/zod-schemas';
	...
	const localStorageForm = localStorageWritable('create-email', initialForm);
...
<div class="container">
	<h1 class="text-3xl font-semibold mb-2">Create email</h1>
	{#if form?.response} 
	...
	<Card.Root class="w-full mb-4">
		<Card.Header>
			<Card.Title>Create new email</Card.Title>
				...
				<div class="grid sm:grid-cols-1 md:grid-cols-1 gap-4">
					<div>
						...
						<Textarea
							...
							bind:value={$formEl.description}
						/>
						<FormFieldError {errors} field="description" />
					</div>
				</div>
