		...
		},
		{
			...
			cell: (item) => {
				return createRender(Actions, {
					id: item.value,
					onEdit: () => {
						const entity = data.countries.find((projType) => projType.id === item.value);
						editObj = {
							id: item.value,
...
<Dialog.Root bind:open={bulkDeleteOpen}>
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			<Dialog.Title>Delete countries</Dialog.Title>
			<Dialog.Description>
...
<Dialog.Root bind:open={addOpen}>
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			<Dialog.Title>Create new country</Dialog.Title>
			<Dialog.Description>Fill in information about new country</Dialog.Description>
		</Dialog.Header>
