		...
		},
		{
			...
			cell: (item) => {
				return createRender(Actions, {
					id: item.value,
					onEdit: () => {
						const entity = data.persons.find((entity) => entity.id === item.value);
						editObj = {
							id: item.value,
...
<Dialog.Root bind:open={bulkDeleteOpen}>
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			<Dialog.Title>Delete people</Dialog.Title>
			<Dialog.Description>
...
<Dialog.Root bind:open={addOpen}>
	<Dialog.Content class="sm:max-w-[425px]">
		<Dialog.Header>
			...
			<Dialog.Description>Fill in information about new person</Dialog.Description>
		</Dialog.Header>
